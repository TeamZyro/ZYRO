...
from ZYROTEAM import YouTube, app
from ZYROTEAM.core.call import ZYRO
from ZYROTEAM.misc import SUDOERS, db
from ZYROTEAM.utils.database import (
    get_active_chats,
    get_lang,
    get_upvote_count,
    is_active_chat,
...
from ZYROTEAM.utils.database import get_assistant
from ZYROTEAM.utils.decorators.language import languageCB
from ZYROTEAM.utils.formatters import seconds_to_min
from ZYROTEAM.utils.inline import close_markup, stream_markup, stream_markup_timer
from ZYROTEAM.utils.stream.autoclear import auto_clean
from ZYROTEAM.utils.thumbnails import get_thumb
from config import (
    BANNED_USERS,
    SOUNCLOUD_IMG_URL,
    STREAM_IMG_URL,
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await ZYRO.pause_stream(chat_id)
        ...
        if command == "Skip":
            ...
            try:
                ...
                if not check:
                    ...
                    try:
                        return await ZYRO.stop_stream(chat_id)
                    except:
                        return
            except:
                try:
        ...
        if "live_" in queued:
            ...
            try:
                ...
                image = None
            try:
                await ZYRO.skip_stream(chat_id, link, video=status, image=image)
            except:
            ...
            try:
                ...
                image = None
            try:
                await ZYRO.skip_stream(chat_id, file_path, video=status, image=image)
            except:
                return await mystic.edit_text(_["call_6"])
            button = stream_markup(_, chat_id)
            img = await get_thumb(videoid)
            ...
            try:
                await ZYRO.skip_stream(chat_id, videoid, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            if videoid == "telegram":
                ...
                try:
                    ...
                    image = None
            try:
                await ZYRO.skip_stream(chat_id, queued, video=status, image=image)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            if videoid == "telegram":
                button = stream_markup(_, chat_id)
