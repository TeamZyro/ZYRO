...
from TEAMZYRO import collection, app, user_collection, require_power, rarity_map 


@app.on_message(filters.command("wdelete"))
...
async def delete_handler(client, message):
    try:
        ...
        if len(args) != 2:
            await message.reply_text("Incorrect format... Please use: /wdelete ID")
            return
...
@app.on_message(filters.command("wupdate"))
@require_power("update_character")
async def update(client: Client, message: Message):
    try:
        args = message.text.split()
        if len(args) != 4:
            await message.reply_text('Incorrect format. Please use: /wupdate id field new_value')
            return
        ...
        if field_to_update in ['name', 'anime']:
            ...
            try:
                ...
                await message.reply_text('Invalid rarity. Please use a valid number for rarity.')
                return
...
async def update_multiple(client: Client, message: Message):
    ...
    16: "⚜ Royal"            
            ...
            try:
                ...
                await message.reply_text('Invalid rarity. Use a valid number for rarity.')
                return
