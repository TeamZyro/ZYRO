...
from TEAMZYRO import user_collection, collection, application, db
...
async def inlinequery(update: Update, context: CallbackContext) -> None:
    ...
    if query.startswith('collection.'):
        ...
        if '.one' in query:
        # Show only characters with vid_url
           all_characters = [character for character in all_characters if 'vid_url' in character]
        else:
        # Show only characters with img_url
           all_characters = [character for character in all_characters if 'vid_url' in character]
    ...
    for character in characters:
        ...
        if 'user' in locals():
            ...
            caption = (
                f"<b>👀😉 look at <a href='tg://user?id={user['id']}'>{escape(user.get('first_name', 'User'))}</a>'s amazing character:</b>\n\n"
        ...
        if 'vid_url' in character:
            ...
            thumbnail_url = character.get('thum_url', 'https://files.catbox.moe/9on5wd.jpg')
            results.append(
                InlineQueryResultVideo(
                    id=f"{character['id']}_{time.time()}",
                    video_url=character['vid_url'],
                    mime_type="video/mp4",
                    thumbnail_url=thumbnail_url,  # Ensure this is not empty
