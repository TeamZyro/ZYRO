...
async def must_join_channel(app: Client, msg: Message):
    if not MUST_JOIN:
        return  # Skip if no channel is specified
    ...
    try:
        ...
        if MUST_JOIN.isalpha():
            join_link = f"https://t.me/{MUST_JOIN}"
        else:
            join_link = f"https://t.me/{MUST_JOIN.lstrip('-')}"

        # Create an inline keyboard button
        keyboard = InlineKeyboardMarkup(
            [[InlineKeyboardButton("Join Here", url=join_link)]]
        )

        # Send the message with button
        await msg.reply(
            "You need to join the channel to use this command:",
            reply_markup=keyboard
        )
        return False  # Indicate the user hasn't joined
    ...
    return True  # Indicate the user has joined
