...
async def update_cookie(client, message):
    ...
    async for msg in userbot.one.get_chat_history(CHANNEL_ID, limit=1):
        if msg.text:
                ...
                if latest_version > current_version:
                    ...
                    async for doc in bot.get_chat_history(CHANNEL_ID, limit=5):
                        if doc.document and latest_file in doc.document.file_name:
                            await userbot.one.download_media(doc.document, file_name=os.path.join("cookies", latest_file))
